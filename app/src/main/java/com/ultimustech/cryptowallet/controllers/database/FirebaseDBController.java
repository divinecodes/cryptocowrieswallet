...
public class FirebaseDBController {
     ...
     * @return uploaded
     */
    public boolean uploadAccount(String accountCode, String accountHash, String uid, String passphrase1,
        ...
        uploadAccountCode(accountHash,accountCode);
        return true;
    }

    public boolean uploadAccountCode(String accountHash,String accountCode){
        firebaseDatabase = FirebaseDatabase.getInstance();
        DatabaseReference codeRef = firebaseDatabase.getReference("codes");

        AccountCode mAccountCode =  new AccountCode(accountHash);
        Map<String,Object> codeUpdate = new HashMap<>();
        codeUpdate.put(accountCode, mAccountCode);

        codeRef.updateChildren(codeUpdate);
        return true;
    }

    public String getAddressByCode(String code){
        final String[] address = {"not found"};


        firebaseDatabase = FirebaseDatabase.getInstance();
        DatabaseReference codeRef = firebaseDatabase.getReference("codes").child(code);
        ...
        ValueEventListener codeListener = new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if(accountCode != null){
                    address[0] = accountCode.accountHash;
                }
            }
